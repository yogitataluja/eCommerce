      ...
      <div className="ntheader_wrapper pr z_200">
        <div className="container">
          <div className="header__bot border_true dn db_lg">
            <nav
              className="nt_navigation tc hover_side_up nav_arrow_false"
              style={{ marginTop: "30px" }}
            >
              {category.map((cate, index) => {
                return (
                  <div className={styles.dropdown} key={index}>
                    <a href={`/shop/${cate.category}`} className={styles.dropbtn}>
                    {cate.category === active? (<strong> {cate.category}</strong>): (cate.category)}
                    {/* {cate.category} */}
                    </a>

                    <div className={styles.dropdowncontent}>
                      {cate.subCategory.map((subcate, index) => (
                        <a href={`/shop/${cate.category}/${subcate.category}`} key={index} >
                          {subcate.category}
                        </a>
                      ))}
                    </div>
